...
import { getProductModel } from "../graphql/queries";
import { updateProductModel } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ProductModelUpdateForm(props) {
  const {
    id: idProp,
    productModel: productModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = productModelRecord
      ? { ...initialValues, ...productModelRecord }
      : initialValues;
    setOwner(cleanValues.owner);
    setProductTitle(cleanValues.productTitle);
    setProductDescription(cleanValues.productDescription);
    setProductType(cleanValues.productType);
    setProductFulfillmentType(cleanValues.productFulfillmentType);
    setProductImages(cleanValues.productImages ?? []);
    setCurrentProductImagesValue("");
    setNetPrice(cleanValues.netPrice);
    setPricingType(cleanValues.pricingType);
    setInventory(cleanValues.inventory);
    setLocation(
      typeof cleanValues.location === "string" || cleanValues.location === null
        ? cleanValues.location
        : JSON.stringify(cleanValues.location)
    );
    setFulfillmentRadius(cleanValues.fulfillmentRadius);
    setGuestOnlyPurchase(cleanValues.guestOnlyPurchase);
    setMinOrderQuantity(cleanValues.minOrderQuantity);
    setMaxOrderQuantity(cleanValues.maxOrderQuantity);
    setMinOrderAmount(cleanValues.minOrderAmount);
    setMaxOrderAmount(cleanValues.maxOrderAmount);
    setMaxGuests(cleanValues.maxGuests);
    setDuration(cleanValues.duration);
    setMinimumNotice(cleanValues.minimumNotice);
    setUnavailableDates(cleanValues.unavailableDates ?? []);
    setCurrentUnavailableDatesValue("");
    setInternalCode(cleanValues.internalCode);
    setUnlimitedInventory(cleanValues.unlimitedInventory);
  ...
  const [productModelRecord, setProductModelRecord] = React.useState(
    productModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getProductModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getProductModel
        : productModelModelProp;
      setProductModelRecord(record);
    };
    queryData();
  }, [idProp, productModelModelProp]);
  React.useEffect(resetStateValues, [productModelRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          productFulfillmentType: productFulfillmentType ?? null,
          productImages: productImages ?? null,
          netPrice: netPrice ?? null,
          pricingType: pricingType ?? null,
          inventory: inventory ?? null,
          location: location ?? null,
          fulfillmentRadius: fulfillmentRadius ?? null,
          guestOnlyPurchase: guestOnlyPurchase ?? null,
          ...
          unlimitedInventory: unlimitedInventory ?? null,
          status: status ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateProductModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: productModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ProductModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || productModelModelProp)}
