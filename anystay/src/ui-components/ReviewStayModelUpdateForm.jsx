...
import { getReviewStayModel } from "../graphql/queries";
import { updateReviewStayModel } from "../graphql/mutations";
export default function ReviewStayModelUpdateForm(props) {
  const {
    id: idProp,
    reviewStayModel: reviewStayModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = reviewStayModelRecord
      ? { ...initialValues, ...reviewStayModelRecord }
      : initialValues;
    setHostawayReview(cleanValues.hostawayReview);
    setHostawayReviewId(cleanValues.hostawayReviewId);
    setExternalUserFirstName(cleanValues.externalUserFirstName);
    setExternalUserLastName(cleanValues.externalUserLastName);
    setOverall(cleanValues.overall);
    setCommunication(cleanValues.communication);
    setAmenities(cleanValues.amenities);
    setCleaning(cleanValues.cleaning);
  ...
  const [reviewStayModelRecord, setReviewStayModelRecord] = React.useState(
    reviewStayModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getReviewStayModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getReviewStayModel
        : reviewStayModelModelProp;
      setReviewStayModelRecord(record);
    };
    queryData();
  }, [idProp, reviewStayModelModelProp]);
  React.useEffect(resetStateValues, [reviewStayModelRecord]);
  const validations = {
    hostawayReview: [],
    hostawayReviewId: [],
    externalUserFirstName: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          hostawayReview: hostawayReview ?? null,
          hostawayReviewId: hostawayReviewId ?? null,
          externalUserFirstName: externalUserFirstName ?? null,
          externalUserLastName: externalUserLastName ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateReviewStayModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: reviewStayModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ReviewStayModelUpdateForm")}
      {...rest}
    >
      <SwitchField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || reviewStayModelModelProp)}
