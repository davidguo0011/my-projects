...
import { getMessageModel } from "../graphql/queries";
import { updateMessageModel } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function MessageModelUpdateForm(props) {
  const {
    id: idProp,
    messageModel: messageModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = messageModelRecord
      ? { ...initialValues, ...messageModelRecord }
      : initialValues;
    ...
    setIsNote(cleanValues.isNote);
    setIsTimestamp(cleanValues.isTimestamp);
    setHostawayMessageId(cleanValues.hostawayMessageId);
    setGuestNoteBody(cleanValues.guestNoteBody);
    setHostNoteBody(cleanValues.hostNoteBody);
    setErrors({});
  };
  const [messageModelRecord, setMessageModelRecord] = React.useState(
    messageModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getMessageModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMessageModel
        : messageModelModelProp;
      setMessageModelRecord(record);
    };
    queryData();
  }, [idProp, messageModelModelProp]);
  React.useEffect(resetStateValues, [messageModelRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          author: author ?? null,
          isNote: isNote ?? null,
          isTimestamp: isTimestamp ?? null,
          hostawayMessageId: hostawayMessageId ?? null,
          guestNoteBody: guestNoteBody ?? null,
          hostNoteBody: hostNoteBody ?? null,
        };
        ...
        try {
          ...
          await API.graphql({
            query: updateMessageModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: messageModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MessageModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Created at"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || messageModelModelProp)}
