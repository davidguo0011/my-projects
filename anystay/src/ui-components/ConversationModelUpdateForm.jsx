...
import { getConversationModel } from "../graphql/queries";
import { updateConversationModel } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ConversationModelUpdateForm(props) {
  const {
    id: idProp,
    conversationModel: conversationModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = conversationModelRecord
      ? { ...initialValues, ...conversationModelRecord }
      : initialValues;
    setUpdatedAt(cleanValues.updatedAt);
    setOwners(cleanValues.owners ?? []);
    setCurrentOwnersValue("");
    setListingmodelID(cleanValues.listingmodelID);
    setExternalConversationId(cleanValues.externalConversationId);
    setExternalSyncEnabled(cleanValues.externalSyncEnabled);
    setExternalLastSyncedAt(cleanValues.externalLastSyncedAt);
    setGuestSeen(cleanValues.guestSeen);
    setHostSeen(cleanValues.hostSeen);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [conversationModelRecord, setConversationModelRecord] = React.useState(
    conversationModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getConversationModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getConversationModel
        : conversationModelModelProp;
      setConversationModelRecord(record);
    };
    queryData();
  }, [idProp, conversationModelModelProp]);
  React.useEffect(resetStateValues, [conversationModelRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          externalSyncEnabled: externalSyncEnabled ?? null,
          externalLastSyncedAt: externalLastSyncedAt ?? null,
          guestSeen: guestSeen ?? null,
          hostSeen: hostSeen ?? null,
          status: status ?? null,
        };
        ...
        try {
          ...
          await API.graphql({
            query: updateConversationModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: conversationModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ConversationModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Updated at"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || conversationModelModelProp)}
