...
import { getRefundRequestModel } from "../graphql/queries";
import { updateRefundRequestModel } from "../graphql/mutations";
export default function RefundRequestModelUpdateForm(props) {
  const {
    id: idProp,
    refundRequestModel: refundRequestModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = refundRequestModelRecord
      ? { ...initialValues, ...refundRequestModelRecord }
      : initialValues;
    ...
    setRefundAmount(cleanValues.refundAmount);
    setRefundReason(cleanValues.refundReason);
    setRefundMessage(cleanValues.refundMessage);
    setStatus(cleanValues.status);
    setErrors({});
  };
  const [refundRequestModelRecord, setRefundRequestModelRecord] =
    React.useState(refundRequestModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getRefundRequestModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRefundRequestModel
        : refundRequestModelModelProp;
      setRefundRequestModelRecord(record);
    };
    queryData();
  }, [idProp, refundRequestModelModelProp]);
  React.useEffect(resetStateValues, [refundRequestModelRecord]);
  const validations = {
    confirmed: [],
    declined: [],
    expired: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateRefundRequestModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: refundRequestModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RefundRequestModelUpdateForm")}
      {...rest}
    >
      <SwitchField
        label="Confirmed"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || refundRequestModelModelProp)}
