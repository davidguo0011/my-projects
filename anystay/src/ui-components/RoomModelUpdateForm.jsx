...
import { getRoomModel } from "../graphql/queries";
import { updateRoomModel } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function RoomModelUpdateForm(props) {
  const {
    id: idProp,
    roomModel: roomModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = roomModelRecord
      ? { ...initialValues, ...roomModelRecord }
      : initialValues;
    setOwner(cleanValues.owner);
    setUserhostmodelID(cleanValues.userhostmodelID);
    setImages(cleanValues.images ?? []);
    setCurrentImagesValue("");
    setRoomTitle(cleanValues.roomTitle);
    setRoomDescription(cleanValues.roomDescription);
    setRoomType(cleanValues.roomType);
    setRoomAmenities(cleanValues.roomAmenities ?? []);
    setCurrentRoomAmenitiesValue("");
    setRoomBeds(cleanValues.roomBeds ?? []);
    setCurrentRoomBedsValue("");
    setIsHotelRoom(cleanValues.isHotelRoom);
  ...
  const [roomModelRecord, setRoomModelRecord] =
    React.useState(roomModelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getRoomModel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRoomModel
        : roomModelModelProp;
      setRoomModelRecord(record);
    };
    queryData();
  }, [idProp, roomModelModelProp]);
  React.useEffect(resetStateValues, [roomModelRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          roomBeds: roomBeds ?? null,
          isHotelRoom: isHotelRoom ?? null,
        ...
        try {
          ...
          await API.graphql({
            query: updateRoomModel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: roomModelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RoomModelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Owner"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || roomModelModelProp)}
