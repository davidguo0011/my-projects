...
export default function PaymentModelUpdateForm(props) {
  const {
    id: idProp,
    paymentModel: paymentModelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = paymentModelRecord
      ? { ...initialValues, ...paymentModelRecord }
      : initialValues;
    ...
    setPaymentIntentID(cleanValues.paymentIntentID);
    setPaymentMethodID(cleanValues.paymentMethodID);
    ...
    setExtraGuestFeePerNight(cleanValues.extraGuestFeePerNight);
    setNights(cleanValues.nights);
    setRefundedAmount(cleanValues.refundedAmount);
    setHostusermodelID(cleanValues.hostusermodelID);
    setGuestusermodelID(cleanValues.guestusermodelID);
    setHoststripeaccountID(cleanValues.hoststripeaccountID);
    setPaymentCreatedAt(cleanValues.paymentCreatedAt);
    setCharges(cleanValues.charges ?? []);
    setCurrentChargesValue("");
    setErrors({});
  };
  const [paymentModelRecord, setPaymentModelRecord] = React.useState(
    paymentModelModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(PaymentModel, idProp)
        : paymentModelModelProp;
      setPaymentModelRecord(record);
    };
    queryData();
  }, [idProp, paymentModelModelProp]);
  React.useEffect(resetStateValues, [paymentModelRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            PaymentModel.copyOf(paymentModelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PaymentModelUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || paymentModelModelProp)}
