...
public class ViewModel {
        ...
        EventHandler<ActionEvent> eventEventHandler =
                new EventHandler<ActionEvent>() {
                    ...
                    public void handle(ActionEvent actionEvent) {
                        for (Terminal eachTerminal : getControlTower()
                                .getTerminals()) {
                            eachTerminal.declareEmergency();
                        }
                        registerChange();
                    }
                };
                    ...
                    public void handle(ActionEvent actionEvent) {
                        if (!(getSelectedAircraft() == null)) {
                            if (getSelectedAircraft().get().getTaskList()
                                    .getCurrentTask().equals(TaskType.LAND)) {
                                try {
                                    suitableGateText.setValue(getControlTower()
                                            .findUnoccupiedGate(
                                                    getSelectedAircraft().get())
                                            .toString());
                                } catch (NoSuitableGateException nge) {
     ...
     * @param queuesWriter             writer to which the takeoff/landing queues and
     *                                 loading map will be written
    ...
    public void saveAs(Writer tickWriter, Writer aircraftWriter,
        ...
        try (BufferedWriter bw = new BufferedWriter(tickWriter)) {
            bw.write(String.valueOf((int) getControlTower().getTicksElapsed()));
        }
        try (BufferedWriter bw = new BufferedWriter(aircraftWriter)) {
            bw.write(String.valueOf(getControlTower().getAircraft().size()));
            if (getControlTower().getAircraft().size() > 0) {
                for (Aircraft eachAircraft : getControlTower().getAircraft()) {
                    bw.newLine();
                    bw.write(eachAircraft.encode());
                }
            }
        }
        try (BufferedWriter bw = new BufferedWriter(queuesWriter)) {
            bw.write(getControlTower().getTakeoffQueue().encode());
            bw.newLine();
            bw.write(getControlTower().getLandingQueue().encode());
            bw.newLine();
            bw.write("LoadingAircraft:" + getControlTower().getLoadingAircraft()
                    .size());
            if (getControlTower().getLoadingAircraft().size() > 0) {
                bw.newLine();
                String loads = "";
                for (Map.Entry<Aircraft, Integer> loadingAircrafts : getControlTower()
                        .getLoadingAircraft().entrySet()) {
                    loads += "" + loadingAircrafts.getKey().getCallsign() + ":"
                            + loadingAircrafts.getValue() + ",";
                }
                loads = loads.substring(0, loads.length() - 1);
                bw.write(loads);
            }
        }
        try (BufferedWriter bw = new BufferedWriter(terminalsWithGatesWriter)) {
            int terminalSize = getControlTower().getTerminals().size();
            bw.write(String.valueOf(terminalSize));
            if (terminalSize > 0) {
                for (int i = 0; i < terminalSize; i++) {
                    bw.newLine();
                    Terminal terminal = getControlTower().getTerminals().get(i);
                    bw.write(terminal.encode());
                }
            }
        }
