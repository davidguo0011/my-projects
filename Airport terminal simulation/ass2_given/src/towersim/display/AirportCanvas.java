...
public class AirportCanvas extends Canvas {
    ...
    private static class ClickableRegion {
         ...
         */
        public ClickableRegion(double x, double y, double width,
                               double height) {
            this.xcoord = x;
            this.ycoord = y;
            this.width = width;
        ...
        public boolean wasClicked(double clickX, double clickY) {
            return clickX >= this.xcoord && clickX <= this.xcoord + this.width
                    && clickY >= this.ycoord
                    && clickY <= this.ycoord + this.height;
        }
    }
    ...
    public AirportCanvas(ViewModel viewModel, double width, double height) {
        ...
        setOnMouseClicked(event -> {
            ...
            for (Map.Entry<ClickableRegion, Aircraft> entry : drawnAircraft
                    .entrySet()) {
        ...
        landTimeline = new Timeline(
                ...
                new KeyFrame(Duration.seconds(1),
                        "end animation",
                        new KeyValue(runwayAnimationX, runwayStartX,
    ...
    public void draw() {
        ...
        drawQueue(viewModel.getControlTower().getLandingQueue(), 0,
                AIRCRAFT_HEIGHT);
        drawAwayAircraft();
        drawTerminals();
        drawTickStatus();
    ...
    private void drawRunway() {
        ...
        for (int i = 0;
             i < ((runwayWidth - runwayTarmacWidth) - lineLength) / lineLength;
             ++i) {
            gc.setStroke(Color.WHITE);
            final double lineY =
                    AIRCRAFT_HEIGHT + marginTop + (runwayHeight / 2);
            final double lineStartOffset = 7; // makes lines look more centered
            gc.strokeLine(runwayStartX + runwayTarmacWidth + lineStartOffset
                            + lineLength / 2
                            + (i * lineLength),
                    lineY,
                    runwayStartX + runwayTarmacWidth + lineStartOffset
                            + lineLength
                            + (i * lineLength),
                    lineY);
        }
    ...
    private void drawQueue(AircraftQueue queue, double x, double y) {
        ...
        gc.fillRect(x, y, AIRCRAFT_WIDTH * queueCapacity + labelWidth,
                AIRCRAFT_HEIGHT);

        gc.setStroke(Color.BLACK);
        gc.strokeRect(x, y, AIRCRAFT_WIDTH * queueCapacity + labelWidth,
                AIRCRAFT_HEIGHT);
        ...
        for (int i = 0; i < aircraft.size(); ++i) {
            ...
            drawAircraft(a, x + labelWidth + AIRCRAFT_WIDTH * i, y,
                    Color.BLACK);
        }
    }
    ...
    private void drawAwayAircraft() {
        ...
        gc.setFill(Color.WHITE);
        gc.fillRect(x, y, AIRCRAFT_WIDTH * capacity + labelWidth,
                AIRCRAFT_HEIGHT);

        gc.setStroke(Color.BLACK);
        gc.strokeRect(x, y, AIRCRAFT_WIDTH * capacity + labelWidth,
                AIRCRAFT_HEIGHT);
    ...
    private void drawTerminals() {
        ...
        final double terminalHeight =
                terminalLabelHeight + terminalAircraftHeight;
        final double marginBelow = 5;
        final double marginLeft = 5;
        final double spaceAbove =
                2 * AIRCRAFT_HEIGHT + 2 * marginBelow; // queues + padding
        final double terminalWidth = getWidth() / 2 - (2 * marginLeft);

        List<Terminal> terminals =
                this.viewModel.getControlTower().getTerminals();
        ...
        for (int i = 0; i < terminals.size(); ++i) {
            ...
            String numGatesText =
                    terminal.getGates().size() + "/" + Terminal.MAX_NUM_GATES
                            + " gates";
            gc.setFill(Color.BLACK);
            gc.setTextBaseline(VPos.CENTER);
            gc.setTextAlign(TextAlignment.LEFT);
     ...
     */
    private void drawAircraft(Aircraft aircraft, double x, double y,
                              Color textColor) {
        GraphicsContext gc = getGraphicsContext2D();

        this.drawnAircraft
                .put(new ClickableRegion(x, y, AIRCRAFT_WIDTH, AIRCRAFT_HEIGHT),
                        aircraft);
